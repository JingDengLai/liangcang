...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                textspan = '<span class="city-picker-span" style="' +
                    ...
                    '<span class="title"></span> ' + (this.options.editable ? '<div class="arrow"></div>' : '' ) + ' </span>',

                dropdown = '<div class="city-picker-dropdown" style="left:0px;bottom:20px;width: 45rem;">' +
                    '<div class="city-select-wrap">' +
                    '<div class="city-select-tab">' +
                    '<a class="active" data-count="province">省份</a>' +
                    (this.includeDem('city') ? '<a data-count="city">城市</a>' : '') +
                    (this.includeDem('district') ? '<a data-count="district">区县</a>' : '') + '</div>' +
                    '<div class="city-select-content" style="height: 260px;">' +
        ...
        feedText: function () {
            ...
            this.$textspan.attr("title", this.getText2());
        },
		...
		setVal: function (code) {
			code = "" + code;
			var province = code.substr(0,2).concat("0000");
			var city = code.substr(0,4).concat("00");
			var district = code;
			...
			for(var i in ChineseDistricts[86]){
				var a = ChineseDistricts[86][i];
				for(var j in a){
					if(a[j].code == province){
						provinceName = a[j].address;
						break;
					}
				}
			}
			if(province){
				this['$province'].data('item',{code:province, address: provinceName});
				if(city){
					this['$city'].data('item',{code:city, address: ChineseDistricts[province][city]});
					if(district){
						this['$district'].data('item',{code:district, address: ChineseDistricts[city][district]});
					}
				}
			}
