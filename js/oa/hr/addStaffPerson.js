...
(function($) {
	...
	'				<h4 class="modal-title" id="addPerson-title">添加人员</h4>' + 
	'			</div>' + 
	...
	function init(option,callback) {
		...
		if($("#addPersonModal").length <= 0){
			...
			$.ajax({
				...
				var options = {
						multiSelect : option?option.multiSelect:true,
	...
	function setSelectName(){
		...
		var ids = selectNodes.map(function(item) {
		    ...
		    //hr-train-发起人
			var type = $("#type").val();
			if("addInitiator" == type){
				var var1 = ids.join(",");
				$('#initiator').val(var1);
				var var2 = names.join(",");
				$('#initiatorName').val(var2);
			}
			//hr-train-参与人员
			if("addStaff" == type){
				var var1 = ids.join(",");
				$('#staffPersonId').val(var1);
				var var2 = names.join(",");
				$('#staffPersonName').val(var2);
			}
			//hr-salary-姓名
			if("addSalaryPerson" == type){
				var var1 = ids.join(",");
				$('#staffId').val(var1);
				var var2 = names.join(",");
				$('#staffName').val(var2);
