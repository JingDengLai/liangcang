        	...
        	if(data){
        		...
        		$.each(data, function (i, item){
					...
					"<div class='storehouse-right storehouse-other temp'>"+
						...
						"<span class='store-span'>虫害数量&nbsp:</span>"+
						"<span>"+(item.pestCount == null? "无":item.pestCount+"只")+"</span><br>"+
						"<span><img onclick='imgCilck(this)' src='images/collect.png' style='width:75px' class='temCode'/><a href='pest/main?groupId="+item.groupId+"&warehouseId="+item.warehouseId+"'><img style='width:100px' src='images/in.png' class='temCode'/></a></span>"+
					"</div>"+
				"</div>";
...
$("#collectBtn").click(function() {
	...
	if(warehouseIds){
		var groupId = $("#groupId").val();
		$.ajax({
			url : "pest/collectPestValue?groupId="+groupId+"&warehouseIds="+warehouseIds,
			type : 'POST'
		}).done(function(data) {
				if(data.status=='200'){
...
function imgCilck(obj){
	...
	var warehouseId = $(obj).parent().parent().parent().find("input[name='warehouseId']").val();
			...
			if(warehouseId){
				$.ajax({
					url : "pest/collectPestValue?groupId="+groupId+"&warehouseIds="+warehouseId,
					type : 'POST'
				}).done(function(data) {
					if(data){
						if(data.status=='200'){
