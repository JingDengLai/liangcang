        	...
        	if(data){
        		...
        		$.each(data, function (i, item){
					...
					"<div class='storehouse-right storehouse-other temp'>"+
						...
						"<span class='store-span'>仓内温度&nbsp:</span>"+
						"<span>"+(item.warehouseTemperature == null? "无":item.warehouseTemperature+"℃")+"</span><br>"+
						"<span class='store-span'>仓内湿度&nbsp:</span>"+
						"<span>"+(item.warehousehumidity == null? "无":item.warehousehumidity+"%")+"</span><br>"+
						"<span class='store-span'>平均粮温&nbsp:</span>"+
						"<span>"+(item.averageTemperature == null? "无":item.averageTemperature+"℃")+"</span><br>"+
						"<span><img onclick='imgCilck(this)' src='images/collect.png' style='width:75px' class='temCode'/><a href='"+location+"?groupId="+item.groupId+"&warehouseId="+item.warehouseId+"'><img style='width:100px' src='images/in.png' class='temCode'/></a></span>"+
					"</div>"+
				"</div>";
...
function hove(){
	$(".collecting").hover(function(){
			var groupId = $("#groupId").val();
			$.ajax({
				url: 'temperature/getCollectStatus',
		        datatype: 'json',
		        type: 'GET',
		        data:{'groupId':groupId},
		        success: function (data) {
...
$("#collectBtn").click(function() {
	...
	if(warehouseIds){
		var groupId = $("#groupId").val();
		$.ajax({
			url : "temperature/collectTempHumidity?groupId="+groupId+"&warehouseIds="+warehouseIds,
			type : 'POST'
		}).done(function(data) {
				if(data.status=='200'){
...
function imgCilck(obj){
	...
	var warehouseId = $(obj).parent().parent().parent().find("input[name='warehouseId']").val();
			...
			if(warehouseId){
				$.ajax({
					url : "temperature/collectTempHumidity?groupId="+groupId+"&warehouseIds="+warehouseId,
					type : 'POST'
				}).done(function(data) {
					if(data){
						if(data.status=='200'){
