        ...
        success: function (data) {
        	if(data){
        		...
        		$.each(data, function (i, item){
                	var html="<div class='temperature' id='"+i+"' onclick='showTemp(this.id)'>"+
					...
					"<div class='storehouse-right storehouse-other temp'>"+
						...
						"<span class='store-span'>磷化氢&nbsp:</span>"+
						"<span>"+(item.averagePh3 == null? "无":item.averagePh3+"ppm")+"</span><br>"+
						"<span class='store-span'>氧气&nbsp:</span>"+
						"<span>"+(item.averageO2 == null? "无":item.averageO2+"%")+"</span><br>"+
						"<span class='store-span'>二氧化碳&nbsp:</span>"+
						"<span>"+(item.averageCo2 == null? "无":item.averageCo2+"ppm")+"</span><br>"+
						"<span><img onclick='imgCilck(this)' src='images/collect.png' style='width:75px' class='temCode'/><a href='gas/main?groupId="+item.groupId+"&warehouseId="+item.warehouseId+"'><img style='width:100px' src='images/in.png' class='temCode'/></a></span>"+
					"</div>"+
				"</div>";
...
$("#collectBtn").click(function() {
	...
	if(warehouseIds){
		var groupId = $("#groupId").val();
		$.ajax({
			url : "gas/collectGasValue?groupId="+groupId+"&warehouseIds="+warehouseIds,
			type : 'POST'
		}).done(function(data) {
				if(data.status=='200'){
...
function imgCilck(obj){
	...
	var warehouseId = $(obj).parent().parent().parent().find("input[name='warehouseId']").val();
			...
			if(warehouseId){
				$.ajax({
					url : "gas/collectGasValue?groupId="+groupId+"&warehouseIds="+warehouseId,
					type : 'POST'
				}).done(function(data) {
					if(data){
						if(data.status=='200'){
	...
	$.ajax({
		...
		if(data && data.length>0){
			...
			$.each(data,function(index,item){
				...
				for(var i = 0; i< $(".storehouse-png").length;i++){
					if($($(".storehouse-png")[i]).attr("id") == warehouseId){
						$($(".storehouse-png")[i]).parent().parent().removeClass("storehouse-border");
						$($(".storehouse-png")[i]).parent().siblings().children(".confirm_arrow").hide();
					}
				}
			});
		}
